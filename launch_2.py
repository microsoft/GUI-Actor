...
NNODE = int(os.environ.get('NODE_COUNT', 1))
NGPUS = int(os.popen('nvidia-smi -L| wc -l').read().split('\n')[0])
...
def parse_args():
    ...
    parser.add_argument("--nnodes", type=int, default=NNODE,
                        help="The number of nodes to use for distributed "
                             "training")
    parser.add_argument("--node_rank", type=int, default=NODE_RANK,
                        help="The rank of the node for multi-node distributed "
                             "training")
    parser.add_argument("--nproc_per_node", type=int, default=NGPUS,
...
def main():
    ...
    # local_rank = int(current_env["LOCAL_RANK"])
    # world_rank = int(current_env["RANK"])
    node_rank = int(current_env["NODE_RANK"])
    ...
    print('Launching Node', node_rank)
    cmd = []
    cmd.append("torchrun")
    cmd.append("--nnodes {}".format(args.nnodes))
    cmd.append("--node_rank {}".format(node_rank))
    cmd.append("--master_addr {}".format(master_addr))
